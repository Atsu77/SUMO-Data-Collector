...
class SumoNetGenerator:
    ...
    def __init__(self, number_of_lanes, road_length):
        self.number_of_lanes = str(number_of_lanes)
        self.road_length = str(road_length)
    ...
    def calculate_lane_y(self, lane_index):
        return round((self.LANE_WIDTH * (int(self.number_of_lanes) - lane_index) - self.LANE_WIDTH / 2) * 100) / 100

    def calculate_junction_y(self):
        return round(self.LANE_WIDTH * int(self.number_of_lanes) * 100) / 100


class SumoRowGenerator:
    def __init__(self, maxSpeed, vehcle_per_hour, begin, end, edge="E0"):
        self.maxSpeed = str(maxSpeed)
        self.vehcle_per_hour = str(vehcle_per_hour)
...
class SumocfgGenerator:
    def __init__(self, net_file, rou_file, begin, end, step_length):
        ...
        self.rou_file = str(rou_file)
        self.begin = str(begin)
        self.end = str(end)
        self.step_length = str(step_length)
        self.current_path = os.path.dirname(os.path.abspath(__file__))
        self.output_path = os.path.join(self.current_path, "../../simulation")
    ...
    def set_output(self, root):
        ...
        fcd_output.set("value", f"../data/fcd_output_{self.step_length}.xml")
